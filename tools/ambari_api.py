...
@mcp.tool()
async def list_cluster_hosts() -> str:
    """
...
@mcp.tool()
async def get_host_details(host_name: str) -> str:
    """
    Retrieves detailed information for a specific host in the Ambari cluster.

    [Tool Role]: Dedicated tool for retrieving comprehensive host details including metrics, hardware info, and components.

    [Core Functions]:
    - Query Ambari REST API for specific host information
    - Return host hardware specs, state, metrics, and assigned components
    - Provide formatted output for LLM automation and cluster management

    [Required Usage Scenarios]:
    - When users request specific host details or host status
    - When auditing or monitoring individual cluster nodes
    - When troubleshooting host-specific issues

    Args:
        host_name: Name of the host to retrieve details for (e.g., "bigtop-hostname0.demo.local")
    ...
    try:
        ...
        result_lines.append(f"  CPU Count: {host_info.get('cpu_count', 'N/A')}")
        result_lines.append(f"  Total Memory: {host_info.get('total_mem', 'N/A')} KB")
        ...
        if disk_info:
            ...
            for i, disk in enumerate(disk_info, 1):
                result_lines.append(f"  Disk {i}:")
                result_lines.append(f"    Mount Point: {disk.get('mountpoint', 'N/A')}")
                result_lines.append(f"    Size: {disk.get('size', 'N/A')} KB")
                result_lines.append(f"    Used: {disk.get('used', 'N/A')} KB ({disk.get('percent', 'N/A')})")
                result_lines.append(f"    Available: {disk.get('available', 'N/A')} KB")
                result_lines.append(f"    Type: {disk.get('type', 'N/A')}")
            result_lines.append("")

        # Host components
        if host_components:
            result_lines.append(f"Host Components ({len(host_components)} components):")
            for i, component in enumerate(host_components, 1):
                host_roles = component.get("HostRoles", {})
                comp_name = host_roles.get("component_name", "Unknown")
                comp_href = component.get("href", "")
                result_lines.append(f"  {i}. Component: {comp_name}")
                result_lines.append(f"     API Link: {comp_href}")
            result_lines.append("")
        else:
        ...
        if metrics:
            result_lines.append("Metrics Available:")
            available_metrics = list(metrics.keys())
            result_lines.append(f"  Available metric categories: {', '.join(available_metrics)}")
            result_lines.append("")
