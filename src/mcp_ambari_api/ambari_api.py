...
async def get_host_details(host_name: Optional[str] = None) -> str:
    ...
    cluster_name = AMBARI_CLUSTER_NAME
    try:
        # If no specific host name provided, get details for all hosts
        if host_name is None:
            # First get list of all hosts
            ...
            result_lines = [f"Detailed Information for All Hosts in Cluster '{cluster_name}' ({len(hosts)} hosts):"]
            ...
            for i, host in enumerate(hosts, 1):
                ...
                # Get detailed info for this specific host
                detail_endpoint = f"/clusters/{cluster_name}/hosts/{current_host_name}"
                detail_response = await make_ambari_request(detail_endpoint)

                if detail_response is None or "error" in detail_response:
                    result_lines.append(f"Error: Unable to retrieve details for host '{current_host_name}'")
                    result_lines.append("")
                    continue

                detail_host_info = detail_response.get("Hosts", {})
                host_components = detail_response.get("host_components", [])
                metrics = detail_response.get("metrics", {})

                # Basic host information
                result_lines.append(f"Host State: {detail_host_info.get('host_state', 'Unknown')}")
                result_lines.append(f"Host Status: {detail_host_info.get('host_status', 'Unknown')}")
                result_lines.append(f"Public Host Name: {detail_host_info.get('public_host_name', 'N/A')}")
                result_lines.append(f"IP Address: {detail_host_info.get('ip', 'N/A')}")
                result_lines.append(f"Maintenance State: {detail_host_info.get('maintenance_state', 'N/A')}")
                result_lines.append(f"OS: {detail_host_info.get('os_type', 'N/A')} ({detail_host_info.get('os_arch', 'N/A')})")
                result_lines.append(f"OS Family: {detail_host_info.get('os_family', 'N/A')}")
                result_lines.append(f"Rack Info: {detail_host_info.get('rack_info', 'N/A')}")

                # Timing information
                last_heartbeat = detail_host_info.get('last_heartbeat_time', 0)
                last_registration = detail_host_info.get('last_registration_time', 0)
                if last_heartbeat:
                ...
                recovery_summary = detail_host_info.get('recovery_summary', 'N/A')
                result_lines.append(f"Recovery: {recovery_summary}")

                # Agent environment summary
                last_agent_env = detail_host_info.get('last_agent_env', {})
                if last_agent_env:
                    ...
                    result_lines.append(f"Live Services: {len(live_services)} ({', '.join([svc.get('name', 'Unknown') for svc in live_services[:3]])}{'...' if len(live_services) > 3 else ''})")
                    result_lines.append(f"Firewall: {'Running' if firewall_running else 'Stopped'}")
                    result_lines.append(f"JCE Policy: {'Unlimited' if has_unlimited_jce else 'Limited'}")

                # Alerts summary
                alerts_summary = detail_response.get('alerts_summary', {})
                if alerts_summary:
                    ...
                    result_lines.append(f"Alerts: {critical} Critical, {warning} Warning, {ok} Ok, {unknown} Unknown")
                ...
                if metrics:
                    ...
                    if cpu_metrics:
                        cpu_count = cpu_metrics.get('cpu_num', detail_host_info.get('cpu_count', 'N/A'))
                        ph_cpu_count = detail_host_info.get('ph_cpu_count', 'N/A')
                        cpu_idle = cpu_metrics.get('cpu_idle', 0)
                        cpu_user = cpu_metrics.get('cpu_user', 0)
                        cpu_system = cpu_metrics.get('cpu_system', 0)
                        result_lines.append(f"  CPU Count: {cpu_count} (Physical: {ph_cpu_count})")
                        result_lines.append(f"  CPU Usage: Idle {cpu_idle}%, User {cpu_user}%, System {cpu_system}%")
                    ...
                    if memory_metrics:
                        ...
                        if mem_total > 0:
                            ...
                            result_lines.append(f"  Total Memory: {mem_total/1024/1024:.1f} GB")
                            result_lines.append(f"  Memory Usage: {mem_used_percent:.1f}% used ({mem_used/1024/1024:.1f}GB/{mem_total/1024/1024:.1f}GB)")
                    
                    # Load average
                    load_metrics = metrics.get('load', {})
                    if load_metrics:
                        load_one = load_metrics.get('load_one', 0)
                        load_five = load_metrics.get('load_five', 0)
                        result_lines.append(f"  Load Average: {load_one} (1m), {load_five} (5m)")
                    
                    # Disk information from metrics
                    ...
                    if disk_metrics:
                        ...
                        if disk_total > 0:
                            ...
                            result_lines.append(f"  Disk Space: {disk_total:.1f} GB total, {disk_used:.1f} GB used ({disk_used_percent:.1f}%)")
                else:
                    # Fallback to basic hardware info if no metrics available
                    cpu_count = detail_host_info.get('cpu_count', 'N/A')
                    ph_cpu_count = detail_host_info.get('ph_cpu_count', 'N/A')
                    total_mem_kb = detail_host_info.get('total_mem', 0)
                ...
                # Host components (summary)
                if host_components:
                    component_names = [comp.get("HostRoles", {}).get("component_name", "Unknown") for comp in host_components]
                    result_lines.append(f"Components: {', '.join(component_names)}")
                else:
                    result_lines.append("Components: None assigned")

                result_lines.append("")

            # Summary
            result_lines.append("SUMMARY:")
            result_lines.append(f"Total Hosts: {len(hosts)}")
            
            return "\n".join(result_lines)

        # If specific host name provided, get details for that host only
        else:
            ...
            result_lines = [f"Host Details for '{host_name}':"]
            result_lines.append("=" * 50)
